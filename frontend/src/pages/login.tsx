...
export default function LoginPage() {
  ...
  return <PageShell>
    ...
    <section className="section"><div className="container auth-layout"><div className="auth-panel"><p className="eyebrow eyebrow--green">Ingreso seguro</p><h2>Continúe con Google.</h2><p>El acceso se valida contra el directorio de colaboradores activos de DIMELCO.</p><button className="button button--primary" type="button" onClick={signIn} disabled={state === 'loading'}>{state === 'loading' ? 'Redirigiendo...' : 'Ingresar con Google'}</button>{state === 'error' && <p className="form-feedback form-feedback--error" role="alert">{message}</p>}</div><aside className="auth-aside"><h3>¿Necesita acceso?</h3><p>Solicite la activación de su cuenta al administrador de la plataforma.</p><Link to="/contacto" className="button button--secondary">Contactar a DIMELCO</Link></aside></div></section>
  </PageShell>;
}
